...
PlotJuggler can be used to analyze ULogs for in-depth, development purposes. It is incredibly useful as every uORB topic is exposed / can be graphed, as well as having custom functions to modify the data (e.g. from Quaternion values to Roll / Pitch / Yaw).
...
The latest version of PlotJuggler for Windows may not work. In this case fallback to v2.8.4 here (this is known to work with Windows).

#### Notes on AppImage for Linux distributions

If the downloaded AppImage does not open, you may need to change its access settings. This is done in the terminal using the following command:sh
...
- Search for vehicle_attitude topic in Timeseries List panel on the left
- Select 4 quaternion members (q.00, q.01, q.02, q.03) by clicking q.00 first, then holding Shift + clicking q.03. They should all be selected
- Click the '+' symbol in the lower-left 'Custom Series' section to create new series
- Select 4 quaternion members again, and drag them to 'Input timeseries + Additional source timeseries' tab on upper-left corner
- Double click on the quat_to_roll from the Function Library. Now you should have the plot on upper section
- Assign the name for the plot (e.g. roll) in the text box in upper-right corner
- Click 'Create New Timeseries'. You should now have the plot in 'Custom Series'
...
The quat_to_roll function looks like this:lua
...
```
 ...
 Edit on GitHubPagerPrevious pageLog Analysis using Flight ReviewNext pageVehicle Status Notifications